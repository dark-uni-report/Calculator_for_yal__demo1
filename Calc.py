...
#для начала работы с графиками нажмите "--graphics--" 
#калькулятор может вылетать при неправильном использовании команд
#отработанны не все возможные ошибки со стороны пользователя
#ввод значений только посредством кнопок
#корень, факториал и тригоном. функции применяются к готовому значению
...
class F(QWidget):
    ...
    def initUI(self):
        ...
        self.btn.clicked.connect(self.r_br)
